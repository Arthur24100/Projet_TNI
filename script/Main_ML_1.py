...
DOSSIERS_DATASET = [
    ...
    "/Users/loeuljeanpierre/Library/Mobile Documents/com~apple~CloudDocs/Documents/2-ESEO/2-2eAnnee/8-TNI/5-Projet/Dataset/1512427/brainTumorDataPublic_1533-2298",
    "/Users/loeuljeanpierre/Library/Mobile Documents/com~apple~CloudDocs/Documents/2-ESEO/2-2eAnnee/8-TNI/5-Projet/Dataset/1512427/brainTumorDataPublic_2299-3064"
]
...
def skull_strip_isolation_cerveau(img_n):
    """
    Skull stripping — pipeline d'isolation du cerveau robuste aux trois vues.

    Stratégie :
      1. Filtrage médian
      2. Seuillage d'Otsu → silhouette de la tête
      3. Remplissage + plus grand CC = tête entière
      4. Détection du scalp brillant dans la bande externe (~4 %)
      5. Dilatation du scalp → couronne (scalp + crâne)
      6. Soustraction tête − couronne ≈ cerveau brut
      7. Plus grand CC le plus central (retire nez/mâchoire)
      8. Dilatation généreuse + intersection zone autorisée
      9. Fermeture finale + remplissage des trous
    """
    # --- Normalisation uint8 (requis par OpenCV et Otsu) ---
    img_f = img_n.astype(np.float32)
    mn, mx = img_f.min(), img_f.max()
    if mx - mn > 1e-8:
        img_uint8 = ((img_f - mn) / (mx - mn) * 255).astype(np.uint8)
    else:
        img_uint8 = np.zeros_like(img_f, dtype=np.uint8)

    img = img_uint8

    # ÉTAPE 1 — Filtrage médian
    from skimage.filters import threshold_otsu as _otsu
    img_filtre = cv2.medianBlur(img, 5)

    # ÉTAPE 2 — Seuillage d'Otsu
    seuil   = _otsu(img_filtre)
    binaire = (img_filtre > seuil).astype(np.uint8)

    # ÉTAPE 3 — Remplissage + plus grand CC = tête entière
    from scipy.ndimage import label as _label
    rempli       = binary_fill_holes(binaire).astype(np.uint8)
    labels_cc, n = _label(rempli)
    if n == 0:
        masque = np.zeros_like(img, dtype=bool)
    ...
    # ÉTAPE 4 — Scalp brillant dans la bande externe (~4 % de la taille)
    epaisseur_bande = max(5, int(0.04 * min(img.shape)))
    ...
    seuil_scalp  = np.percentile(pixels_tete, 70) if len(pixels_tete) > 0 else 255
    scalp        = ((img_filtre >= seuil_scalp) & (bande_externe > 0)).astype(np.uint8)

    # ÉTAPE 5 — Dilatation du scalp → couronne
    ...
    rayon_erosion_loc = max(2, int(0.006 * min(img.shape)))
    elt_erosion       = cv2.getStructuringElement(
        cv2.MORPH_ELLIPSE, (2 * rayon_erosion_loc + 1, 2 * rayon_erosion_loc + 1))
    erode             = cv2.erode(cerveau_brut, elt_erosion, iterations=1)

    labels_cc2, n2 = _label(erode)
    if n2 == 0:
        ...
        cerveau_seul = (labels_cc2 == int(np.argmax(scores))).astype(np.uint8)

    # ÉTAPE 8 — Dilatation généreuse + intersection zone autorisée
    rayon_dilatation = rayon_erosion_loc + rayon_couronne + 2
    ...
    # ÉTAPE 9 — Fermeture finale + remplissage
    elt_fermeture = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 9))
    masque_cerv   = cv2.morphologyEx(masque_cerv, cv2.MORPH_CLOSE, elt_fermeture, iterations=2)
    masque_cerv   = binary_fill_holes(masque_cerv).astype(np.uint8)

    masque       = masque_cerv.astype(bool)
