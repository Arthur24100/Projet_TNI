"""
Script de test du skull stripping avec Watershed.
- Seed au centre de l'image = cerveau
- S'arrête aux gradients forts = os du crâne
"""

import h5py, numpy as np, os
import matplotlib.pyplot as plt
from skimage.morphology import closing, disk
from skimage.segmentation import watershed
from skimage.filters import sobel
from skimage.measure import label, regionprops
from scipy.ndimage import binary_fill_holes, gaussian_filter
from skimage.filters import threshold_otsu
from skimage.morphology import erosion

import numpy as np

# ─── PARAMÈTRES À AJUSTER ────────────────────────────────────
DOSSIER_BASE = "/Users/loeuljeanpierre/Library/Mobile Documents/com~apple~CloudDocs/Documents/2-ESEO/2-2eAnnee/8-TNI/5-Projet/Dataset/1512427/brainTumorDataPublic_1-766"

FICHIERS_TEST = [
    "511.mat",
    "552.mat",
    "559.mat",
]

SIGMA = 10  # lissage gaussien avant watershed — augmente pour moins de bruit
SEED_RADIUS = 30  # taille du seed central en pixels


# ─────────────────────────────────────────────────────────────

def normaliser(img):
    return (img - img.min()) / (img.max() - img.min() + 1e-8)

def skull_strip_watershed(img_n, sigma=SIGMA, seed_radius=SEED_RADIUS):
    H, W = img_n.shape

    img_lisse = gaussian_filter(img_n, sigma=sigma)
    thresh = threshold_otsu(img_lisse)
    masque_binaire = img_lisse > thresh * 0.5

    masque_binaire = closing(masque_binaire, disk(10))
    masque_binaire = binary_fill_holes(masque_binaire)

    etiq = label(masque_binaire)
    props = regionprops(etiq)
    if not props:
        return img_n.astype(np.float32), np.ones((H, W), dtype=bool)
    masque_tete = etiq == max(props, key=lambda p: p.area).label
    masque_tete = binary_fill_holes(masque_tete)

    prop = max(regionprops(label(masque_tete)), key=lambda p: p.area)
    min_row, min_col, max_row, max_col = prop.bbox
    hauteur = max_row - min_row
    largeur = max_col - min_col

    rayon_erosion = max(4, int(min(hauteur, largeur) * 0.06))
    masque_cerveau = erosion(masque_tete, disk(rayon_erosion))

    gradient = sobel(img_lisse)
    p10 = np.percentile(img_n, 10)
    ys, xs = np.where(masque_cerveau)
    cy = int(ys.mean()) if len(ys) > 0 else H // 2
    cx = int(xs.mean()) if len(xs) > 0 else W // 2

    markers = np.zeros((H, W), dtype=np.int32)
    r = seed_radius
    markers[max(0, cy - r):min(H, cy + r),
            max(0, cx - r):min(W, cx + r)] = 1
    markers[img_n < p10 * 0.5] = 2
    markers[~masque_tete] = 2

    labels_ws = watershed(gradient, markers, mask=masque_tete)
    masque_ws = (labels_ws == 1)
    masque_ws = closing(masque_ws, disk(5))
    masque_ws = binary_fill_holes(masque_ws)

    masque_final = masque_ws & masque_cerveau
    masque_final = binary_fill_holes(masque_final)

    # ✨ NOUVEAU : garder UNIQUEMENT le composant connexe
    # qui contient le point seed (centre du cerveau)
    # → colonne vertébrale et tissus déconnectés disparaissent naturellement
    etiq_final = label(masque_final)
    label_seed = etiq_final[cy, cx]  # label du composant qui contient le seed

    if label_seed != 0:
        # Garder uniquement ce composant
        masque_final = (etiq_final == label_seed)
    else:
        # Fallback : prendre le plus grand composant
        props_final = regionprops(etiq_final)
        if props_final:
            masque_final = (etiq_final == max(props_final, key=lambda p: p.area).label)

    masque_final = binary_fill_holes(masque_final)
    masque_final = closing(masque_final, disk(3))

    return (img_n * masque_final).astype(np.float32), masque_final


# ─── Chargement + skull stripping ────────────────────────────
donnees = []
for chemin_rel in FICHIERS_TEST:
    chemin = os.path.join(DOSSIER_BASE, chemin_rel)
    nom = os.path.basename(chemin)
    print(f"Traitement : {nom}...")

    try:
        with h5py.File(chemin, 'r') as f:
            img = np.array(f['cjdata']['image']).T.astype(np.float32)
            mask_gt = (np.array(f['cjdata']['tumorMask']).T > 0)

        img_n = normaliser(img)
        img_stripped, masque = skull_strip_watershed(img_n)

        rgb = np.stack([img_n] * 3, axis=-1).copy()
        rgb[masque, 1] = np.clip(rgb[masque, 1] + 0.4, 0, 1)  # vert = cerveau
        rgb[mask_gt, 0] = np.clip(rgb[mask_gt, 0] + 0.5, 0, 1)  # rouge = tumeur GT

        pct = 100 * (mask_gt & masque).sum() / (mask_gt.sum() + 1e-8)
        donnees.append((nom, img_n, img_stripped, rgb, pct))
        print(f"  → tumeur conservée : {pct:.0f}%")

    except FileNotFoundError:
        print(f"  Fichier non trouvé : {chemin}")
        donnees.append((nom, None, None, None, 0))
    except Exception as e:
        print(f"  Erreur : {e}")
        donnees.append((nom, None, None, None, 0))

# ─── Figure 1 : images 1 à 4 ─────────────────────────────────
fig1, axes1 = plt.subplots(3, 3, figsize=(12, 16))
fig1.suptitle(f"Figure 1/2 — Skull Stripping Watershed  sigma={SIGMA}  seed={SEED_RADIUS}px",
              fontsize=11, fontweight="bold")

for i, (nom, img_n, img_s, rgb, pct) in enumerate(donnees[:4]):
    if img_n is None:
        for j in range(3): axes1[i, j].axis('off')
        axes1[i, 0].set_title(f"{nom} — NON TROUVÉ", fontsize=9)
        continue
    axes1[i, 0].imshow(img_n, cmap='gray');
    axes1[i, 0].set_title(f"{nom} — Original", fontsize=9);
    axes1[i, 0].axis('off')
    axes1[i, 1].imshow(img_s, cmap='gray');
    axes1[i, 1].set_title("Cerveau isolé (Watershed)", fontsize=9);
    axes1[i, 1].axis('off')
    axes1[i, 2].imshow(rgb);
    axes1[i, 2].set_title(f"Overlay — {pct:.0f}% conservée", fontsize=9);
    axes1[i, 2].axis('off')
plt.tight_layout()

plt.show()